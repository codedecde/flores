...
PREFIX=CC_ALIGNED_ALL_DATA
LANG_DATA=$DATA/all-clean-$TGT
mkdir -p $LANG_DATA

if [ ! -e $LANG_DATA/cc-aligned.train.$SRC ]; then
    ...
    awk 'BEGIN {FS="\t"} {print $1}' $SHUFFLED_TRAIN > $LANG_DATA/cc-aligned.train.$SRC
    awk 'BEGIN {FS="\t"} {print $3}' $SHUFFLED_TRAIN > $LANG_DATA/cc-aligned.train.$TGT

    echo "Creating validation data..."
    awk 'BEGIN {FS="\t"} {print $1}' $SHUFFLED_VALID > $LANG_DATA/cc-aligned.valid.$SRC
    awk 'BEGIN {FS="\t"} {print $3}' $SHUFFLED_VALID > $LANG_DATA/cc-aligned.valid.$TGT
